...
app = FastAPI(title="Hugo User Service with RBAC", version="2.0.0")
...
def require_role(allowed_roles: List[UserRole]):
    def decorator(func):
        async def wrapper(*args, **kwargs):
            ...
            if not current_user or current_user['role'] not in [role.value for role in allowed_roles]:
                raise HTTPException(
...
def require_same_company_or_hugo_manager(func):
    async def wrapper(*args, **kwargs):
        current_user = kwargs.get('current_user')
        target_company_id = kwargs.get('company_id')
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            """, (
                ...
                company.max_users, company.contact_person, company.contact_email,
                company.contact_phone, company.address
            ))
...
async def health_check():
    return {"status": "healthy", "service": "user-service-rbac", "version": "2.0.0"}
